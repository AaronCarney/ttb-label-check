...
**The same Worker holds the service warm.** Cloud Run runs at `--min-instances 0`, so an idle
instance is reclaimed and the next visitor pays a container start plus an OCR model load. Measured
