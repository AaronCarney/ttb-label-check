...
millimetres needs the image's physical scale, which a photograph does not carry. `type_size_min` as
...
mandatory label information regarding type size, characters per inch, or contrasting background". A
prototype that rejects labels on three measurements the agency does not routinely review would be
wrong about the job.
...
upload; that is a larger piece of work than the brief asks for.
...
so the figure the rules need does not exist anywhere in its inputs.
...
`spirits`, because nothing on a bottle reliably distinguishes a wine from a spirit and nothing ever
asked them to try. Until now the evaluator replaced that tag only when the application named a
beverage, so a wine label filed without an application inherited `spirits` and was checked against the
spirits rules. That is the one failure this product cannot have: the spirits rules include the
standards of identity, and a wine label reading `TABLE WHITE WINE` is not one of Part 5 Subpart I's
classes, so the app would have reported a compliant wine label as failing a rule that never applied to
...
`applies_to_classes` and the engine selects a rule only where a reading's class is in that list. The
eight health-warning rules list all three beverages and every other rule lists exactly one, so the
warning rules are already independent of the beverage in substance. What stops them running is the
mechanism: `FieldObservation.beverage_class` is a required three-valued field with no way to say "not
stated", so a reading has to claim a beverage before any rule will look at it, including the eight
...
**Rejected.** *Refuse the upload outright, with a 400 and "pick the beverage type"* — two tests
deliberately accept an image on its own, so the product has already decided that an image alone is a
...
directions. *Repair the projection here* — the repair needs the reader to return the label's alcohol
statement as printed, a change to `app/vision/local.py` and `app/vision/cloud.py`, whose work is
gathered in one pass against frozen OCR output and whose proof cannot be made without an OCR run.
...
already has. This check is unbuilt, not unbuildable. *Lower the severity to `warn`* — it sends every
label whose alcohol statement the reader could not place to a human, with a reason code naming a
formatting defect nobody examined. *Pass when nothing was read* — the same claim of an unmade check
that [0006](#0006) refused for contrast and type size. *Delete `format_check.py` along with the
rules* — `app/rules/loader.py:211` refuses startup when any rule names a validator the registry does
not carry, and it makes that check for disabled rules too.
...
`Beer` itself so no table entry is unreachable. `Rose Wine` comes off wine's recognised list.
...
label. They do not require it to be one of a short set.
...
designation "champagne" "may appear in lieu of the class designation 'sparkling wine'", so a label
reading CHAMPAGNE and an application reading SPARKLING WINE name the same wine. With Champagne merely
...
rejected label. Rose Wine does not belong and is removed: §4.21(a)(4) puts "pink (or rose) wine" beside
...
carrying "rose wine" also carries "wine", which is listed.

**Rejected.** *Extend the `present` allow-lists until they agree with each other* — the smallest change
and it fixes the two spirits labels, but it leaves the cause in place and cannot fix wine at all.
...
rule is configured. *Drop the `present` rules altogether, since `matches_application` also looks at the
...
| Rule | Citation | Reason code |
|---|---|---|
| `common.warning.contrasting_bg` | §16.22(a)(1) | `WARNING.LEGIBILITY.CONTRAST_NOT_MEASURED` |
| `common.warning.cpi_max` | §16.22(a)(4) | `WARNING.TYPE_SIZE.CPI_NOT_MEASURED` |
| `common.warning.type_size_min` | §16.22(b) | `WARNING.TYPE_SIZE.HEIGHT_NOT_MEASURED` |
| `common.warning.separate_apart` | §16.21 | `WARNING.PLACEMENT.ISOLATION_NOT_MEASURED` |
...
fixed 2.0, while §16.22(b) sets the minimum in millimetres keyed to container size, so a pass from that
comparison did not mean what its own citation says. Code whose answer is wrong in a way its citation
...
`insufficient_evidence` at warn severity under `WARNING.STYLE.BOLD_NOT_MEASURED`.
`app/vision/heading_measure.py` no longer falls back to measuring the lower half of the whole image when
...
confident. Against a reject-severity rule, treating "not measured" as "not bold" turns the product's own
...
does not exist, each carrying a wrong verdict for whoever switches it on. *One shared "not measured"
reason code* — four rules ask four different questions, and a reviewer reads the code's description as a
sentence. *Delete the §16.22(a)(4) decision table with `cpi_lookup`* — the table is the regulation's
three normative rows as data; it costs nothing and it is what a characters-per-inch check reads the day a
...
- **The floor: 0.633%.** Converting every authorized size to fluid ounces and rounding to a tenth — the
  precision the labels in the manifest actually print, which gives 50 mL as 1.7 FL OZ and 375 mL as 12.7
  FL OZ — the worst case is 0.550%, at 50, 100, 200 and 250 mL. A label may also print the tenth *below*
...
- **The ceiling: 1.216%.** Take each authorized size and the customary figure printed for the nearest
  ...
  above that would let a 720 mL label pass against a 710 mL application.
...
(§5.70(a), §7.70(a), §4.37) but prescribes no precision for them and publishes no equivalents table. If
...
version pins. A constant in `quantity_match.py` would be neither.
...
admits anything within half a pint and an application declaring 700 mL passes against a pint label.
*A table of admissible equivalents* — the most explainable design, and it cannot be completed, because
...
writing that the label carries that name. The whole-words route is the same test [0007](#0007) settled
...
since Jaro is invariant when both strings are reversed. *Resolve `ttb-26233001000566` through the
trade-name list alone* — its trade name is `UGLY WINES` and the label mark is `THE UGLY`; the best score
against the whole admissible set is 0.88, short of the 0.92 a match needs. *Put the State-name table in
...
- **A punctuation-only difference reports a match.** `Lucky Lucy's` against an application's `Lucky Lucys`
  ...
  says it passes with no other outcome acceptable. The deciding source is TTB's own: Form 5100.31's
  ...
  punctuation is kept, a punctuation difference produces a measured score graded by how much of the name
  the difference is, and a message the reviewer sees.
- **A city name can corroborate a State.** The State fold is applied to both sides, so an over-fold is
  symmetric. What remains is that a label naming a city could corroborate an application naming the same
...
"PRODUCT OF LITHUANIA", "DISTILLED IN IRELAND". Where the label states an origin and it does not carry
...
than a built check that reports a wrong verdict. *Keep rejecting on an unrecognised origin statement* — it
rejects compliant imports, and TTB approved labels that state their origin in a form the check does not
read. *Pass an unrecognised origin statement* — it reports a check as made and passed when the two sides
were never lined up. *Translate with the model reader* — [0009](#0009) removed the model reasoning layer,
