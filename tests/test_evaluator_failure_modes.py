...
@pytest.mark.asyncio
@pytest.mark.parametrize("reason_code, outcome, case_label", [
    ...
    ("CLASS_TYPE.MATCH.APPLICATION_LABEL_DISAGREE",   Outcome.INSUFFICIENT_EVIDENCE, "class-type-disagreement"),
...
async def test_registry_reason_codes_route_to_needs_review(reason_code, outcome, case_label):
    ...
    rules = FakeRuleEngine(results=(
        ValidationResult(
            rule_id=f"R-{case_label}", cfr_citation="27 CFR §x",
