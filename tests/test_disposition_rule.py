...
def _warn_fail():
    ...
    return ValidationResult(
        rule_id="R-warn", cfr_citation="27 CFR §x", beverage_class=BeverageClass.SPIRITS,
        outcome=Outcome.FAIL, severity=Severity.WARN, reason_code="X.MATCH.NEEDS_REVIEW",
...
def _reject_fail():
    return ValidationResult(
        rule_id="R-reject", cfr_citation="27 CFR §x", beverage_class=BeverageClass.SPIRITS,
        outcome=Outcome.FAIL, severity=Severity.REJECT, reason_code="X.MATCH.DISAGREE",
        aggregated_confidence=0.9, engine_meta=_em(),
    )
