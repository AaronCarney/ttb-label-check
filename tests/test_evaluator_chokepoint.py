...
async def test_vision_exception_routes_to_needs_review(caplog):
    ...
    assert any(
        getattr(r, "reason_code", None) == "ENGINE.EXTRACTION.UNAVAILABLE" for r in caplog.records
    )
...
async def test_rule_engine_exception_routes_to_needs_review(caplog):
    ...
    assert envelope.disposition == "needs_review"
    assert any(
        getattr(r, "reason_code", None) == "ENGINE.RULES.UNAVAILABLE" for r in caplog.records
    )
