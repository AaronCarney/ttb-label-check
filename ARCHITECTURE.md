...
| `app/api/` | HTTP surface. `labels.py` evaluates one submission; `batches.py` accepts a batch and streams its results; `ui/` serves the browser-facing pages, one module per job behind one router — `shells.py` the page shells, `single_upload.py` and `bulk_upload.py` the two upload paths, `images.py` the uploaded label image, `samples.py` the shipped sample labels; `healthz.py` reports readiness; `overrides.py` records an agent's correction to one field's result or decision on the whole label, and recomputes the label's result from it. |
